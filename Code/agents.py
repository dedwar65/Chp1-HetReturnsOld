...
class CstwMPCMarket(Market):  # EstimationMarketClass
    ...
    def __init__(self, **kwds):
        ...
        # Save the current file's PARENT directory location for writing output:
        self.my_file_path = os.path.join(here,"..")
    ...
    def show_many_stats(self, spec_name=None):
        ...
        if spec_name is not None:
            with open(
                self.my_file_path + "/Paper/Results/" + spec_name + "Results.txt",
                "w",
                encoding="utf-8",
            ) as f:
